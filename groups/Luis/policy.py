...
class BaseLuisAgent(Policy):
    ...
    def act(self, s: np.ndarray) -> int:
        ...
        for col in valid:
            ...
            if self.opponent_weight > 0.0:
                scores[col] -= self.opponent_weight * self.opponent_answer(next_board, player)

        best_score = max(scores.values())
        best_cols = [col for col in self.center_order if col in scores and scores[col] == best_score]
        return best_cols[0]
    ...
    def features(self, board: np.ndarray, player: int) -> dict[str, float]:
        ...
        center_col = board[:, 3]
        features["center"] = float(np.sum(center_col == player) - np.sum(center_col == -player))
