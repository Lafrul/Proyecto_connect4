...
CSV_PATH = ROOT / "s_vs_laura_50_results.csv"
...
for starter, group in [("Todos", games), *games.groupby("starter")]:
    ...
    wins_laura = int((group["winner_agent"] == LAURA_NAME).sum())
    ...
    summary_rows.append(
        {
            ...
            "games": total,
            "s_wins": wins_s,
            "laura_wins": wins_laura,
            "draws": draws,
            "s_win_rate": wins_s / total,
            "laura_win_rate": wins_laura / total,
...
summary.to_csv(OUT_DIR / "s_vs_laura_50_summary.csv", index=False)

# Grafica 10: resultados globales y separados por quien inicia.
fig, axes = plt.subplots(1, 2, figsize=(15, 6))

overall = summary[summary["starter"] == "Todos"].iloc[0]
labels = ["S heuristico", "Laura MCTS", "Empate"]
values = [overall["s_wins"], overall["laura_wins"], overall["draws"]]
colors = [COLORS[S_NAME], COLORS[LAURA_NAME], COLORS["Draw"]]
...
for col, label, color in [
    ...
    ("laura_win_rate", "Gana Laura", COLORS[LAURA_NAME]),
...
fig.suptitle("Grafica 10. S vs Laura: resultados alternando inicio", fontsize=15, fontweight="bold")
save(fig, "10_s_vs_laura_resultados.png")

# Grafica 11: costo computacional y duracion.
fig, axes = plt.subplots(1, 2, figsize=(15, 6))

time_df = df.copy()
display_names = {
    S_NAME: "S heuristico",
    LAURA_NAME: "Laura MCTS",
}
time_df["agent_label"] = time_df["focal_agent"].map(display_names)

data = [
    time_df[time_df["focal_agent"] == S_NAME]["mean_decision_time"].to_numpy(),
    time_df[time_df["focal_agent"] == LAURA_NAME]["mean_decision_time"].to_numpy(),
]
box = axes[0].boxplot(
    data, tick_labels=["S heuristico", "Laura MCTS"], patch_artist=True
)
for patch, color in zip(box["boxes"], [COLORS[S_NAME], COLORS[LAURA_NAME]]):
...
axes[1].plot(games["game_id"] + 1, games["total_game_time"], color="#264653", linewidth=2, label="Tiempo total")
axes[1].scatter(games["game_id"] + 1, games["total_game_time"], color="#264653", s=30)
axes_moves = axes[1].twinx()
axes_moves.plot(games["game_id"] + 1, games["num_moves"], color="#d95f02", linewidth=2, alpha=0.75, label="Movimientos")
...
fig.suptitle("Grafica 11. S vs Laura: costo computacional", fontsize=15, fontweight="bold")
save(fig, "11_s_vs_laura_tiempos.png")
